...
class Agent:
	def __init__(self, state_size, is_eval=False):
		...
		if os.path.exists('models/target_model'):
			self.policy_net = torch.load('models/policy_model1', map_location=device)
			self.target_net = torch.load('models/target_model1', map_location=device)
		else:
			self.policy_net = DQN(state_size, self.action_size)
			self.target_net = DQN(state_size, self.action_size)
		self.optimizer = optim.Adamax(self.policy_net.parameters(), lr=0.005)
	...
	def act(self, state):
		...
		tensor1 = torch.FloatTensor(state).to(device)
		options = self.target_net(tensor1)
	...
	def optimize(self):
		...
		#print((next_state_values * self.gamma))
		#print(reward_batch)
		# Compute Huber loss
